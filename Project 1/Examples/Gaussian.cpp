...
int gauss3x3at( cv::Mat &src, cv::Mat &dst ) {
    ...
    // // alternative
    // dst.create( src.size(), src.type() ); // makes a duplicate size image, uninitialized
    ...
    for(int i = 1; i<src.rows-1; i++) { // rows
        for(int j=1;j<src.cols-1;j++) { // columns
            for(int k=0;k<src.channels();k++) { // color channels
                // implement filter for each pixel
...
int gauss3x3ptr( cv::Mat &src, cv::Mat &dst ) { // pass images by reference
    ...
    for(int i=1;i<src.rows-1;i++) { // rows
        ...
        cv::Vec3b *ptrup = src.ptr<cv::Vec3b>(i-1); // pointer to the data for row i-1
        cv::Vec3b *ptrmd = src.ptr<cv::Vec3b>(i); // pointer to the data for row i
        cv::Vec3b *ptrdn = src.ptr<cv::Vec3b>(i+1); // pointer to the data for row i+1
        cv::Vec3b *dptr = dst.ptr<cv::Vec3b>(i); // result image data for row i

        for(int j=1;j<src.cols-1;j++) { // columns
            for(int k=0;k<src.channels();k++) { //channels
